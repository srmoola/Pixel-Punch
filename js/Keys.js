export const keys = {
  ...
  w: {
    // Since there is gravity, when the player jumps it will slowly start to fall.
    pressed: false,
  },
  // Enemy.
  ArrowLeft: {
  ...
  ArrowUp: {
    // Since there is gravity, when the enemy jumps it will slowly start to fall.
    pressed: false,
  },
};
...
export function loadKeyDownEvents(player, enemy) {
  ...
  window.addEventListener("keydown", (event) => {
    ...
    switch (event.key) {
      ...
      case "ArrowUp":
        if (!enemy.inTheAir) {
          // Only can jump if it's not in the air.
          enemy.velocity.y = -enemy.moveFactor * 4;
        }
        break;
      case "Shift": // Enemy attack with Shift key.
